...
2. The SEAL skill entrypoint exists and describes the supported invocation.
3. `seal-invoke` can run on a fixture repository copied to a temporary directory.
4. The invocation writes `.seal/map.yaml`, `.seal/debt.yaml`, `.seal/impacts/IMPACT-initial.yaml`, `.seal/proof.yaml`, `.seal/evidence/index.yaml`, and `.seal/reports/gap-review.md`.
5. `seal-validate` library validation passes on the generated artifact set.
