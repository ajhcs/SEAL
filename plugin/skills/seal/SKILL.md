...
The supported Codex invocation is skill-based: ask Codex to use the SEAL skill on a plan file, gstack-style Markdown output, or repository path. For local validation and smoke tests, use the short RC workflow command: `seal guide <directory|plan.md> [change target] [summary]`, `seal repo map <directory>`, `seal plan ingest <plan.md>`, `seal impact <directory> <target> [summary]`, `seal proof <directory>`, `seal launch <directory>`, and `seal validate <directory>`. `seal guide` writes a beginner-facing guide report with validation-backed next steps while keeping canonical records in `.seal/*.yaml` artifacts. `seal repo map` writes starter artifacts, ingestion gaps, and rendered map views for a repository. `seal plan ingest` writes starter artifacts for a Markdown plan in its containing directory.
