...
function hasVisibleGap(fileRecord, mapGapIds, debtRecords = []) {
  if ((fileRecord.gap_refs ?? []).some((gapId) => mapGapIds.has(gapId))) {
    return true;
  }
...
function componentHasCoverage(component, files, componentDebtRecords = []) {
  ...
  if (files.some((file) => file.component_id === component.id)) {
    return true;
  }
...
export async function validateFileCoverage(rootPath, { map = {}, debt = {} } = {}) {
  ...
  const mapGapIds = new Set((map.gaps ?? []).map((gap) => gap.id));
  ...
  for (const [filePath, observedClassification] of inventory) {
    ...
    if (!fileRecord) {
      ...
      errors.push({
        ...
        fix: "Regenerate the map or add this file with classification, source_refs, and either component_id or an explicit visible gap.",
    ...
    const hasComponent = Boolean(fileRecord.component_id && componentIds.has(fileRecord.component_id));
    ...
    if (!hasComponent && !hasGap) {
      errors.push({
        ...
        actual: "no component_id and no visible gap/debt record",
  ...
  const coveredComponents = map.components ?? [];
